...
func main() {
	...
	natsClient, err := natsCli.NewCli(*natsURL, opts)
	if err != nil {
		log.Fatalf("nats client: %v\n", err)
	}

	mdlw := natsCli.NewCoreChain(metricsMiddleware)

	srv := reg.Service{}
	h := errorCatchWrapper(reg.NewHandler(srv, natsClient.Publish))
	if err := natsClient.ReplyFunc(*subj, mdlw.Then(h)); err != nil {
		log.Fatalf("nats reply func for %s: %v", *subj, err)
	}

	// Add NATS status checks.
	natsConnCheck := healthcheck.Check(func() error {
		if status := natsClient.Status(); status != nats.CONNECTED {
...
func errorCatchWrapper(h handleFunc) natsCli.CoreHandler {
	wrp := func(ctx context.Context, msg *nats.Msg) {
...
func metricsMiddleware(next natsCli.CoreHandler) natsCli.CoreHandler {
	h := func(ctx context.Context, msg *nats.Msg) {
